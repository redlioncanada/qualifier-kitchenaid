...
! function(a) {
    "use strict";
    a.module("ngSlider", []).directive("slider", ["$compile", "$templateCache", "$timeout", "$window", "slider",
        function(b, c, d, e, f) {
            return {
                restrict: "AE",
                require: "?ngModel",
                    ...
                    if (j) {
                        ...
                        var l = !1,
                            m = function() {
                                ...
                                b.calculate = g.options.calculate || void 0, b.onstatechange = g.options.onstatechange || void 0, g.slider = g.slider ? g.slider.init(h, g.tmplElt, b) : p(h, g.tmplElt, b), l || k();
                                var c = g.tmplElt.find("div")[7];
                                a.element(c).html(g.slider.generateScale()), g.slider.drawScale(c), g.ngDisabled && o(g.ngDisabled), l = !0
                            };
                        j.$render = function() {
...
function(a) {
    "use strict";
    a.module("ngSlider").factory("sliderUtils", ["$window",
        function(a) {
            return {
                offset: function(a) {
                    var b = a[0],
...
function(a) {
    "use strict";
    a.module("ngSlider").factory("sliderDraggable", ["sliderUtils",
        function(b) {
            function c() {
...
function(a) {
    "use strict";
    a.module("ngSlider").factory("sliderPointer", ["sliderDraggable", "sliderUtils",
        function(b, c) {
            function d() {
                b.apply(this, arguments)
            }
            return d.prototype = new b, d.prototype.oninit = function(b, c, d, e) {
                this.uid = c, this.parent = e, this.value = {}, this.vertical = d, this.settings = a.copy(e.settings), this.normalOrder = this.parent.settings.from < this.parent.settings.to, this.threshold = this.parent.settings.threshold
                ...
                var c = this._getPageCoords(b);
                this._set(this.calc(this.vertical ? c.y : c.x)), this.settings.realtime && this.settings.cb && a.isFunction(this.settings.cb) && this.allowed && this.settings.cb.call(this.parent, this.parent.getValue(), !this.is.drag)
            }, d.prototype.onmouseup = function() {
                this.settings.cb && a.isFunction(this.settings.cb) && this.allowed && this.settings.cb.call(this.parent, this.parent.getValue(), !this.is.drag), this.is.drag || this.ptr.removeClass("jslider-pointer-hover")
            ...
            }, d.prototype._set = function(a) {
                if (this.allowed = !0, this.value.origin = this.parent.prcToValue(a), this.threshold && this.parent.o.pointers[1]) {
                    var b = this.parent.o.pointers[0].value.origin,
                        c = this.parent.o.pointers[1].value.origin;
                    this.allowed = (this.normalOrder ? c - b : b - c) >= this.threshold
                }
                if (!this.allowed) {
                    var d = this.parent.o.pointers[0 === this.uid ? 1 : 0];
                    return void(this.value.origin = 0 === this.uid ? this.normalOrder ? d.value.origin - this.threshold : d.value.origin + this.threshold : this.normalOrder ? d.value.origin + this.threshold : d.value.origin - this.threshold)
                }
                this.value.prc = a, this.ptr.css(this.vertical ? {
                    top: a + "%",
                    marginTop: -5
                } : {
                    left: a + "%"
...
function(a) {
    "use strict";
    a.module("angularAwesomeSlider").factory("slider", ["sliderPointer", "sliderConstants", "sliderUtils",
        function(b, c, d) {
            function e() {
            ...
            return e.prototype.init = function(b, d, e) {
                return this.settings = e, this.inputNode = b, this.inputNode.addClass("ng-hide"), this.settings.interval = this.settings.to - this.settings.from, this.settings.calculate && a.isFunction(this.settings.calculate) && (this.nice = this.settings.calculate), this.settings.onstatechange && a.isFunction(this.settings.onstatechange) && (this.onstatechange = this.settings.onstatechange), this.css = c.SLIDER.css, this.is = {
                    init: !1
                }, this.o = {}, this.initValue = {}, this.create(d), this
                ...
                var t = [h, i],
                    u = e.settings.from < e.settings.to ? !0 : !1,
                    v = e.settings.value.split(";");
                a.forEach(t, function(c, f) {
                    e.settings = a.copy(e.settings);
                    var g = v[f];
                    if (g) {
                        e.o.pointers[f] = new b(c, f, e.settings.vertical, e);
                        var h = v[f - 1],
                            i = h ? parseInt(h, 10) : void 0;
                        g = e.settings.round ? parseFloat(g) : parseInt(g, 10), (h && u ? i > g : g > i) && (g = h);
                        var j = u ? g < e.settings.from : g > e.settings.from,
                            k = j ? e.settings.from : g;
                        j = u ? g > e.settings.to : g < e.settings.to, k = j ? e.settings.to : g, e.o.pointers[f].set(k, !0);
                        var l = d.offset(e.o.pointers[f].ptr);
                        e.o.pointers[f].d = {
                            left: l.left,
                            top: l.top
                        }
                    }
                }), e.domNode.bind("mousedown", e.clickHandler.apply(e)), this.o.value = a.element(this.domNode.find("i")[2]), this.is.init = !0, this.settings.css && (l.css(this.settings.css.background ? this.settings.css.background : {}), this.o.pointers[1] || (n.css(this.settings.css.before ? this.settings.css.before : {}), q.css(this.settings.css.after ? this.settings.css.after : {})), o.css(this.settings.css["default"] ? this.settings.css["default"] : {}), p.css(this.settings.css["default"] ? this.settings.css["default"] : {}), m.css(this.settings.css.range ? this.settings.css.range : {}), h.css(this.settings.css.pointer ? this.settings.css.pointer : {}), i.css(this.settings.css.pointer ? this.settings.css.pointer : {})), a.forEach(this.o.pointers, function(a) {
                    e.redraw(a)
                })
                ...
                var b = this,
                    ...
                    e = function() {
                        var a = b.o.pointers[0].ptr,
                            c = b.o.pointers[1].ptr,
                            e = d.offset(a),
                            f = d.offset(c);
                        b.o.pointers[0].d = {
                            left: e.left,
                            top: e.top,
                            width: a[0].clientWidth,
                ...
                return function(f) {
                    if (!b.disabled) {
                        var g = b.settings.from < b.settings.to,
                            h = 0,
                            i = d.offset(b.domNode),
                            j = b.o.pointers[0],
                            k = b.o.pointers[1] ? b.o.pointers[1] : null;
                        if (!g && k) {
                            var l = k;
                            k = j, j = l
                        }
                        var m = f.originalEvent ? f.originalEvent : f,
                            n = c ? m.pageY : m.pageX,
                            o = {
                                left: i.left,
                                top: i.top,
                        ...
                        var v = j._getPageCoords(f);
                        return p.cx = v.x - p.d.left, p.cy = v.y - p.d.top, p.onmousemove(f, v.x, v.y), p.onmouseup(), a.extend(p.d, {
                            left: v.x,
                            top: v.y
                        }), b.redraw(p), !1
                    }
                ...
                if (this.o.pointers[0] && !this.o.pointers[1]) {
                    ...
                    this.o.indicators[3].css(c >= 0 ? this.settings.vertical ? {
                        height: c + "%"
                    } : {
                        width: c + "%"
                ...
                function b(a, b, d) {
                    ...
                    if (c.sizes.domWidth) {
                        ...
                        0 > f && (b.margin -= f), c.sizes.domWidth > 0 && b.border + b.label / 2 > e ? (b.margin = 0, b.right = !0) : b.right = !1
                    }
                    return a.o.css(c.settings.vertical ? {
                        top: d + "%",
                        marginLeft: "20px",
                        bottom: "auto"
                    } : {
                        ...
                        right: "auto"
                    }), b.right && c.sizes.domWidth > 0 && a.o.css(c.settings.vertical ? {
                        top: d + "%",
                        bottom: 0
                    } : {
                        left: d + "%",
                ...
                var f, g, h = 0 === d.o[0].offsetWidth ? 7 * d.o[0].textContent.length : d.o[0].offsetWidth,
                    i = {
                        label: c.settings.vertical ? d.o[0].offsetHeight : h,
                        right: !1,
                        border: e * (c.settings.vertical ? this.sizes.domHeight : this.sizes.domWidth) / 100
                    }, j = 0 === a.uid ? 1 : 0;
                if (!this.settings.single && !this.settings.vertical) {
                    f = this.o.labels[j], g = this.o.pointers[j];
                    var k = this.o.labels[0],
                        l = this.o.labels[1],
                        m = this.o.pointers[0],
                        n = this.o.pointers[1];
                    k.o.css(this.css.visible), l.o.css(this.css.visible);
                    var o = n.ptr[0].offsetLeft - m.ptr[0].offsetLeft;
                    o + 10 < k.o[0].offsetWidth + l.o[0].offsetWidth ? (true) : (f.value.html(this.nice(g.value.origin)), f.o.css(this.css.visible))               }
                i = b(d, i, e);
                var p = c.settings.vertical ? c.sizes.domHeight : c.sizes.domWidth;
                if (f) {
                    var q = 0 === d.o[0].offsetWidth ? d.o[0].textContent.length / 2 * 7 : d.o[0].offsetWidth,
                        r = {
                            label: c.settings.vertical ? f.o[0].offsetHeight : q,
                            right: !1,
...
function(a) {
    "use strict";
    a.module("ngSlider").run(["$templateCache",
        function(a) {
            a.put("ng-slider/slider-bar.tmpl.html", '<span ng-class="mainSliderClass" id="{{sliderTmplId}}"><table><tr><td><div class="jslider-bg"><i class="l"></i><i class="r"></i><i class="v"></i><i class="p"></i><i class="s"></i><i class="s"></i><i class="f"></i></div><div class="jslider-pointer"></div><div class="jslider-pointer jslider-pointer-to"></div><div class="jslider-label"><span>{{options.from}}</span>{{options.dimension}}</div><div class="jslider-label jslider-label-to"><span>{{to}}</span>{{options.dimension}}</div><div class="jslider-value"><span></span>{{options.dimension}}<div>None</div></div><div class="jslider-value jslider-value-to"><span></span>{{options.dimension}}</div><div class="jslider-scale" id="{{sliderScaleDivTmplId}}"></div></td></tr></table></span>')
        }
    ])
}(window.angular);
